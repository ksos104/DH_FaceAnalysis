...
class CriterionCrossEntropyEdgeParsing_boundary_attention_loss(nn.Module):
    ...
    def forward(self, preds, target):
        target[0] = target[0].squeeze(dim=1)         # segments
        target[1] = target[1].squeeze(dim=1)         # edge
        ...
        loss = self.loss_weight[0]*loss_parse + self.loss_weight[1]*loss_edge + self.loss_weight[2]*loss_att_edge
